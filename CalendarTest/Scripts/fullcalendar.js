	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (event.distribution) {
		    event.distribution.forEach(function (item) {
		        if (item.slots !=null && item.slotsTotal != null) {
		            availableSlots += item.slotsTotal - item.slots;
                }
		    });
		}

		var title = '<div class="fc-title" style="margin-right: 50px">' +
                    '<div style="float: left; width: 100%">' +
                    htmlEscape(event.title) +
                    '</div>' +
                    '<div style="float: right; width: 50px; margin-right: -50px; text-align: right">' +
                    (availableSlots > 0 ? htmlEscape(availableSlots) : htmlEscape('0')) +
                    '</div>' +
		...
		titleHtml =
			'<span class="fc-title">' +
				//(htmlEscape(event.title + ' slots: ' + event.slots + ' of ' + event.slotsTotal || '') || '&nbsp;') + // we always want one line of height
